...
const spawn = (script, arguments_, options) =>
  nanoSpawn('node', [script, ...arguments_], options).then(subprocess => ({
    ...subprocess,
    exitCode: 0,
  }));

// Avoids "ExperimentalWarning: Importing JSON modules is an experimental feature and might change at any time"
const importWithTypeJson = async file => JSON.parse(await fsPromises.readFile(path.resolve(import.meta.dirname, file), 'utf8'));

const packageJson = await importWithTypeJson('../package.json');
const errorPattern = /(\.md|\.markdown|\.mdf|stdin):\d+(:\d+)? error MD\d{3}/gv;
...
test('linting of incorrect Markdown file fails', async t => {
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--config', 'test-config.json', 'incorrect.md']));
  t.is(error.stdout, '');
...
test('linting of incorrect Markdown file fails and prints issues', async t => {
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--config', 'warning-config.json', 'incorrect.md']));
  t.is(error.stdout, '');
...
test('linting of incorrect Markdown file fails and prints issues as json', async t => {
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--config', 'warning-config.json', 'incorrect.md', '--json']));
  t.is(error.stdout, '');
...
test('linting of incorrect Markdown file fails with absolute path', async t => {
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--config', 'test-config.json', path.resolve('incorrect.md')]));
  t.is(error.stdout, '');
  t.is(error.stderr.match(errorPattern).length, 7);
  t.is(error.exitCode, 1);
});
...
test('linting of unreadable Markdown file fails', async t => {
  ...
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--config', 'test-config.json', unreadablePath]));
...
test('glob linting works with failing files', async t => {
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--config', 'test-config.json', '**/*.md']));
  t.is(error.stdout, '');
...
test('dir linting works with failing .markdown files', async t => {
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--config', 'test-config.json', 'subdir-incorrect']));
  t.is(error.stdout, '');
...
test('dir linting works with failing .markdown files and absolute path', async t => {
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--config', 'test-config.json', path.resolve('subdir-incorrect')]));
  t.is(error.stdout, '');
  t.is(error.stderr.match(errorPattern).length, 9);
  t.is(error.exitCode, 1);
});
...
test('glob linting with failing files has fewer errors when ignored by dir', async t => {
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--config', 'test-config.json', '**/*.md', '--ignore', 'subdir-incorrect']));
  t.is(error.stdout, '');
...
test('glob linting with failing files has fewer errors when ignored by dir and absolute path', async t => {
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--config', 'test-config.json', '**/*.md', '--ignore', path.resolve('subdir-incorrect')]));
  t.is(error.stdout, '');
  t.is(error.stderr.match(errorPattern).length, 8);
  t.is(error.exitCode, 1);
});
...
test('dir linting with failing files has fewer errors when ignored by file', async t => {
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--config', 'test-config.json', 'subdir-incorrect', '--ignore', 'subdir-incorrect/incorrect.md']));
  t.is(error.stdout, '');
...
test('dir linting with failing files has fewer errors when ignored by file and absolute path', async t => {
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--config', 'test-config.json', path.resolve('subdir-incorrect'), '--ignore', 'subdir-incorrect/incorrect.md']));
  t.is(error.stdout, '');
  t.is(error.stderr.match(errorPattern).length, 2);
  t.is(error.exitCode, 1);
});
...
test('glob linting does not try to lint directories as files', async t => {
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--config', 'test-config.json', '**/*', '--ignore', '**/*.mdf']));
  t.is(error.stdout, '');
...
test('--stdin with invalid input reports violations', async t => {
  const stdin = {string: ['Heading', '', 'Text ', ''].join('\n')};
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--stdin'], {stdin}));
  t.is(error.stdout, '');
  t.is(error.stderr.match(errorPattern).length, 2);
  t.is(error.exitCode, 1);
});
...
test('--output with invalid input outputs violations', async t => {
  ...
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--stdin', '--output', output], {stdin}));
  t.is(error.stdout, '');
  t.is(error.stderr, '');
  t.is(error.exitCode, 1);
...
test('--output with invalid input and --json outputs issues as json', async t => {
  ...
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--stdin', '--output', output, '--json'], {stdin}));
  t.is(error.stdout, '');
  t.is(error.stderr, '');
  t.is(error.exitCode, 1);
...
test('--output with invalid path fails', async t => {
  ...
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--stdin', '--output', output], {stdin}));
  t.is(error.stdout, '');
  t.is(error.stderr.replace(/: ENOENT[\s\S]*$/v, ''), 'Cannot write to output file ' + output);
...
test('linting using a toml configuration file works', async t => {
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--config', 'test-config.toml', '**/*.md']));
  t.is(error.stdout, '');
  t.is(error.stderr.match(errorPattern).length, 15);
  t.is(error.exitCode, 1);
});
...
test('linting using a yaml configuration file works', async t => {
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--config', 'test-config.yaml', '**/*.md']));
  t.is(error.stdout, '');
  t.is(error.stderr.match(errorPattern).length, 15);
  t.is(error.exitCode, 1);
});
...
test('error on configuration file not found', async t => {
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--config', 'non-existent-file-path.yaml', 'correct.md']));
  t.is(error.stdout, '');
  t.regex(error.stderr, /Cannot read or parse config file 'non-existent-file-path\.yaml': ENOENT: no such file or directory, open '.*non-existent-file-path\.yaml'/v);
  t.is(error.exitCode, 4);
});

test('error on malformed YAML configuration file', async t => {
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--config', 'malformed-config.yaml', 'correct.md']));
  t.is(error.stdout, '');
  t.regex(error.stderr, /Cannot read or parse config file 'malformed-config.yaml': Unable to parse 'malformed-config.yaml'; Parser 0:/v);
  t.is(error.exitCode, 4);
});
...
test('invalid JSON Pointer', async t => {
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--config', 'nested-config.json', '--configPointer', 'INVALID', '**/*.md']));
  t.is(error.stdout, '');
  t.regex(error.stderr, /Invalid JSON pointer\./v);
  t.is(error.exitCode, 4);
});

test('empty JSON Pointer', async t => {
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--config', 'nested-config.json', '--configPointer', '/EMPTY', 'incorrect.md']));
  t.is(error.stdout, '');
  t.is(error.stderr.match(errorPattern).length, 7);
  t.is(error.exitCode, 1);
});
...
test('valid JSON Pointer with JSON configuration', async t => {
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--config', 'nested-config.json', '--configPointer', '/key', 'incorrect.md']));
  t.is(error.stdout, '');
...
test('valid JSON Pointer with TOML configuration', async t => {
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--config', 'nested-config.toml', '--configPointer', '/key/subkey', 'incorrect.md']));
  t.is(error.stdout, '');
...
test('Custom rule from single file loaded', async t => {
  ...
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--rules', 'custom-rules/files/test-rule-1.cjs', '--stdin'], {stdin}));
...
test('Multiple custom rules from single file loaded', async t => {
  const stdin = {string: '# Input\n'};
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--rules', 'custom-rules/files/test-rule-3-4.cjs', '--stdin'], {stdin}));
...
test('Custom rules from directory loaded', async t => {
  const stdin = {string: '# Input\n'};
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--rules', 'custom-rules/files', '--stdin'], {stdin}));
...
test('Custom rules from glob loaded', async t => {
  const stdin = {string: '# Input\n'};
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--rules', 'custom-rules/files/**/*.cjs', '--stdin'], {stdin}));
  const expected = ['stdin:1 error test-rule-1 Test rule broken', 'stdin:1 error test-rule-2 Test rule 2 broken', 'stdin:1 error test-rule-3 Test rule 3 broken', 'stdin:1 error test-rule-4 Test rule 4 broken'].join('\n');
  t.is(error.stdout, '');
  t.is(error.stderr, expected);
  t.is(error.exitCode, 1);
...
test('Custom rule from node_modules package loaded', async t => {
  const stdin = {string: '# Input\n'};
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--rules', 'markdownlint-cli-local-test-rule', '--stdin'], {stdin}));
...
test('Custom rule from package loaded', async t => {
  const stdin = {string: '# Input\n'};
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--rules', './custom-rules/markdownlint-cli-local-test-rule', '--stdin'], {stdin}));
  const expected = ['stdin:1 error markdownlint-cli-local-test-rule Test rule package broken'].join('\n');
  t.is(error.stdout, '');
  t.is(error.stderr, expected);
  t.is(error.exitCode, 1);
...
test('Custom rule from several packages loaded', async t => {
  const stdin = {string: '# Input\n'};
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--rules', './custom-rules/markdownlint-cli-local-test-rule', '--rules', './custom-rules/markdownlint-cli-local-test-rule-other', '--stdin'], {stdin}));
...
test('Invalid custom rule name reports error', async t => {
  const stdin = {string: '# Input\n'};
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--rules', 'markdownlint-cli-local-test-rule', '--rules', 'invalid-package', '--stdin'], {stdin}));
...
test('fixing errors in a file yields fewer errors', async t => {
  ...
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--fix', '--config', 'test-config.json', fixFileA]));
...
test('fixing errors in a file with absolute path yields fewer errors', async t => {
  ...
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--fix', '--config', 'test-config.json', path.resolve(fixFileB)]));
  t.is(error.stdout, '');
  t.is(error.stderr.match(errorPattern).length, 1);
  t.is(error.exitCode, 1);
...
test('fixing errors with a glob yields fewer errors', async t => {
  ...
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--fix', '--config', 'test-config.json', fixFileGlob]));
  t.is(error.stdout, '');
  t.is(error.stderr.match(errorPattern).length, 2);
  t.is(error.exitCode, 1);
...
test('--ignore-path fails for missing file', async t => {
  ...
  t.is(error.stdout, '');
  t.regex(error.stderr, /enoent.*no such file or directory/iv);
  t.is(error.exitCode, 1);
});

test('Linter text file --output must end with EOF newline', async t => {
  const output = '../outputE.txt';
  const endOfLine = new RegExp(os.EOL + '$', 'v');
  await t.throwsAsync(() => spawn('../markdownlint.js', ['--config', 'test-config.json', '--output', output, 'incorrect.md']));
  t.regex(fs.readFileSync(output, 'utf8'), endOfLine);
  fs.unlinkSync(output);
});

test('--dot option to include folders/files with a dot', async t => {
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--config', 'test-config.json', '--dot', '**/incorrect-dot.md', '**/.file-with-dot.md', '**/correct.md']));
  t.is(error.stdout, '');
...
test('with --quiet option does not print to stdout or stderr', async t => {
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--quiet', '--config', 'test-config.json', 'incorrect.md']));
  t.is(error.stdout, '');
  t.is(error.stderr, '');
  t.is(error.exitCode, 1);
});
...
test('--enable flag', async t => {
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--enable', 'MD041', '--config', 'default-false-config.yml', 'incorrect.md']));
  t.is(error.stdout, '');
...
test('--enable flag does not modify already enabled rules', async t => {
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--enable', 'MD043', '--config', 'md043-config.yaml', 'correct.md']));
  t.is(error.stdout, '');
...
test('--enable flag accepts rule alias', async t => {
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--enable', 'first-line-heading', '--config', 'default-false-config.yml', 'incorrect.md']));
  t.is(error.stdout, '');
  t.is(error.stderr, 'incorrect.md:1 error MD041/first-line-heading/first-line-h1 First line in a file should be a top-level heading [Context: "## header 2"]');
  t.is(error.exitCode, 1);
});
...
test('--disable flag', async t => {
  ...
  const error = await t.throwsAsync(() => spawn('../markdownlint.js', ['--disable', 'MD014', 'MD014', 'MD022', '--', 'incorrect.md']));
  t.is(error.stdout, '');
  t.is(error.stderr, 'incorrect.md:1 error MD041/first-line-heading/first-line-h1 First line in a file should be a top-level heading [Context: "## header 2"]');
  t.is(error.exitCode, 1);
});
